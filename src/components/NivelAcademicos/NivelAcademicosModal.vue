                    ...
                    <v-row>
                        <v-col cols="12">
                            <v-text-field v-model="formData.nombre"
                                          label="Nombre del Nivel Acad�mico"
...
<script setup>
    ...
    const tituloModal = computed(() => (props.modoEditar ? 'Editar Nivel Acad�mico' : 'Nuevo Nivel Acad�mico'));

    const rules = {
        required: v => !!v || 'Este campo es requerido.',
        maxLength: length => v => !v || v.length <= length || `M�ximo ${length} caracteres.`
    };
    ...
    const guardar = async () => {
        ...
        if (valid) {
            ...
            try {
                ...
                errorForm.value = error.message || 'Ocurri� un error al guardar.';
                console.error(error);
            } finally {
