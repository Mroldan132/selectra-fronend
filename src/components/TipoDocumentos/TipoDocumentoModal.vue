      ...
      <v-card-text class="pa-5">
        <v-form ref="formRef">
          ...
          <v-row>
            <v-col cols="12">
              <v-text-field
                v-model="formData.nombreTipoDocumento"
...
const props = defineProps({
  visible: Boolean,
  tipoDocumentoParaEditar: Object,
  modoEditar: Boolean,
});
...
const tituloModal = computed(() => (props.modoEditar ? 'Editar Tipo de Documento' : 'Nuevo Tipo de Documento'));
...
const guardar = async () => {
  ...
  if (valid) {
    ...
    try {
      if (props.modoEditar) {
        await TipoDocumentoService.actualizarTipoDocumento(formData);
      } else {
        await TipoDocumentoService.crearTipoDocumento(formData);
      }
      emit('guardado');
    } catch (error) {
...
watch(() => props.visible, (newVal) => {
  ...
  if (newVal) {
    ...
    if (props.modoEditar && props.tipoDocumentoParaEditar) {
        Object.assign(formData, props.tipoDocumentoParaEditar);
    } else {
