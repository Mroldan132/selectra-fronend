          ...
          <v-row>
            <v-col cols="12">
              <v-text-field
                ...
                label="Nombre del Tipo de Pregunta"
...
const tituloModal = computed(() => (props.modoEditar ? 'Editar Tipo de Pregunta' : 'Nuevo Tipo de Pregunta'));
...
const guardar = async () => {
  ...
  if (valid) {
    ...
    try {
      if (props.modoEditar) {
        await TipoPreguntaService.actualizarTipoPregunta(formData);
      } else {
        await TipoPreguntaService.crearTipoPregunta(formData);
      }
      emit('guardado');
    } catch (error) {
