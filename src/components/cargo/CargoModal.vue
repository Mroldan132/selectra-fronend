...
const tituloModal = computed(() => (props.modoEditar ? 'Editar Cargo' : 'Nuevo Cargo'));
...
const guardar = async () => {
  ...
  if (valid) {
    ...
    try {
      if (props.modoEditar) {
        await CargoService.actualizarCargo(formData.cargoId, formData);
      } else {
        await CargoService.crearCargo(formData);
      }
      emit('guardado');
    } catch (error) {
    ...
    errorForm.value = 'Por favor completa todos los campos correctamente.';
  }
};
...
watch(() => props.visible, (newVal) => {
  ...
  if (newVal) {
    errorForm.value = '';
    formRef.value?.resetValidation();
    if (props.modoEditar && props.cargoParaEditar) {
      Object.assign(formData, props.cargoParaEditar);
    } else {
      Object.assign(formData, getInitialData());
    }
  }
