<template>
    <v-container fluid>
        <v-card class="elevation-2 rounded-lg">
            <v-toolbar flat color="white">
                <v-toolbar-title class="font-weight-bold text-grey-darken-2">
                    ...
                    Gestión de Tipos de Pregunta Filtro
                </v-toolbar-title>
            ...
            <v-data-table :headers="headers"
                ...
                <template v-slot:item.actions="{ item }">
                    ...
                    <v-icon small color="red" @click="eliminarTipo(item)" title="Eliminar">
                        mdi-delete
                    </v-icon>
                </template>
            </v-data-table>
        ...
        <TipoPreguntaModal :visible="dialogVisible"
...
<script setup>
    ...
    const handleGuardado = () => {
        ...
        const message = modoEditar.value
            ? 'Tipo de pregunta filtro actualizado correctamente.'
            : 'Tipo de pregunta filtro creado correctamente.';
        showSnackbar(message);
        fetchTiposPregunta();
    };

    const eliminarTipo = async (item) => {
        if (confirm(`¿Está seguro de eliminar el tipo de pregunta "${item.nombre}"?`)) {
            try {
                await TipoPreguntasFiltroService.eliminarTipoPregunta(item.tipoPreguntaId);
                showSnackbar('Tipo de pregunta filtro eliminado correctamente.');
                fetchTiposPregunta();
            } catch (error) {
                console.error("Error al eliminar el tipo de pregunta:", error);
                showSnackbar(error.message || 'Error al eliminar el tipo de pregunta filtro.', 'error');
            }
        }
    };
